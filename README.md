...
>
> 🎬 完整宣传片(27s)：[docs/promo.mp4](docs/promo.mp4)
...
4. **自动启动网易云音乐**（如果还没开）——会探测常见安装位置，找不到可在 config.toml 里设 `netease_path`。
5. **自动等待网易云并连上** —— 先开桥还是先开网易云都行；网易云暂停时自动补静音，流不断。
...
正确用法：
1. 从 mod 的**官方页面**下载并解压 mod（里面有 `version.dll`、`spotify-radio/`、`media/`）：
...
- **延迟越来越大**：v0.1.2 已用“实时时钟补静音”修掉了延迟累积。若仍觉得整体偏慢，那是
  mod 端电台的固定缓冲，属正常的几秒级。
- **两路声音重叠 / 回声 / 还能直接听到网易云 / 采集跟着 Windows 音量变**：
  根因是网易云在**直接外放**。解决办法（**最简单**）：
  **网易云音乐 → 设置 → 播放 → 把“输出设备”选一个不发声的设备**
  （如没接音箱的 HDMI 声卡、或 NVIDIA Virtual Audio Device）。
  备选：Windows 设置 → 系统 → 声音 → 音量混合器 → 网易云音乐 → 输出设备 → 选一个没在用的设备。
  程序里运行 `route` 会自动打开这个 Windows 设置页。这样：
  - 你的主扬声器里**听不到**网易云了（只通过游戏电台、按场景门控出声）；
  - 你在主扬声器上调音量**不再影响**采集（采集跟的是那个独立设备，保持 100% 即可）；
  - 把网易云 App 自身音量也开到 100%，最终响度用游戏内电台音量控制。
